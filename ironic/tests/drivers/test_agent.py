...
class TestAgentDeploy(db_base.DbTestCase):
    ...
    def test_deploy_with_power_off(self, power_mock, bootdev_mock):
        with task_manager.acquire(
                self.context, self.node['uuid'], shared=True) as task:
            task.node.power_state = states.POWER_OFF
            driver_return = self.driver.deploy(task)
            self.assertEqual(driver_return, states.DEPLOYWAIT)
            bootdev_mock.assert_called_once_with(task, 'pxe')
    ...
    def test_take_over(self, update_neutron_mock):
        ...
        with task_manager.acquire(
            ...
            update_neutron_mock.assert_called_once_with(
                task, CONF.agent.pxe_bootfile_name)
    ...
    def test_tear_down(self, power_mock, bootdev_mock, neutron_mock):
        with task_manager.acquire(
            ...
            bootdev_mock.assert_called_with(task, 'pxe', persistent=True)
            neutron_mock.assert_called_with(task.context)

        self.assertEqual(driver_return, states.DECOMMISSIONING)
